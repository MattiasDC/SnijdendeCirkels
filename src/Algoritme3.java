...
public class Algoritme3 extends Algoritme{
	...
	public void run(ArrayList<Cirkel> cirkels) {
		...
		TreeSet<Cirkel> queue = new TreeSet<Cirkel>(new Comparator<Cirkel>() {

			@Override
			public int compare(Cirkel c1, Cirkel c2) {
				return Double.compare(c1.getMiddelpunt().getY(), c2.getMiddelpunt().getY());
			}
		});

		TreeSet<Cirkel> stralen = new TreeSet<Cirkel>(new Comparator<Cirkel>() {

			@Override
			public int compare(Cirkel c1, Cirkel c2) {
				return -Double.compare(c1.getStraal(), c2.getStraal());
			}
		});
		...
		for (Event evenement : evenementen){
			if (evenement.links){
				for (Cirkel cirkel : queue.subSet(new Cirkel(new Punt(evenement.x,evenement.cirkel.getTop()-maxStraal),0D),new Cirkel(new Punt(evenement.x,evenement.cirkel.getBottom()+maxStraal),0D))){
					if(cirkel.getBottom() >= evenement.cirkel.getTop() && cirkel.getTop() <= evenement.cirkel.getBottom())
